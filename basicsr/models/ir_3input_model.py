...
class IR3InputModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(IR3InputModel, self).__init__(opt)

        # define network
        self.net_g = define_network(deepcopy(opt['network_g']))
        self.net_g = self.model_to_device(self.net_g)

        opt_clip = deepcopy(opt['network_clip'])
        self.net_clip = CLIPDeRouter(opt_clip.pop('vision_tower'), opt_clip.pop('cls_num'))
        self.net_clip.to(dtype=torch.bfloat16)
        ckpt = torch.load(opt_clip.pop('ckpt'), map_location='cpu')['params']
        self.net_clip.load_state_dict(ckpt)
        self.net_clip = self.model_to_device(self.net_clip)
    ...
    def init_training_settings(self):
        self.net_g.train()
        lora.mark_only_lora_as_trainable(self.net_g, bias='all')
        for param in self.net_g.parameters():
            param.requires_grad = True
        self.net_clip.eval()
        for param in self.net_clip.parameters():
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            probs, de_prior = self.net_clip(self.lq_clip)
            probs = probs.float()
            de_prior = de_prior.float()
        preds = self.net_g(self.lq, de_prior, probs)
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                    probs, de_prior = self.net_clip(self.lq_clip[i:j])
                    probs = probs.float()
                    de_prior = de_prior.float()
                pred = self.net_g(self.lq[i:j],de_prior,probs)
